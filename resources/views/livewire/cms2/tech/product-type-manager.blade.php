...
<div class="w-[600px] mx-auto">
    ...
    <form wire:submit.prevent="addProductType">
        ...
        <div>
            ...
            <input type="number" min="1" max="99" step="1" wire:model="order" id="order" class="border rounded p-2" required>
        ...
        <button type="submit" class="bg-blue-500 text-white rounded px-4 py-2">Добавить тип изделия</button>
    </form>

    <h3 class="mt-6 bg-gray-200 p-2 mb-3">Список типов изделий</h3>

    <ul>
        @foreach($productTypes as $productType)
            <li class="flex justify-between items-center bg-white mb-1">
                {{ $productType->name }} ({{ $productType->types }}) {{ $productType->order }}
                <button wire:confirm="Удалить ?" wire:click="deleteProductType({{ $productType->id }})" class="bg-red-500 text-white rounded px-2 py-1">Удалить</button>
            </li>
        @endforeach
    </ul>
</div>
